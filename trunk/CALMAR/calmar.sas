...
CONTPOI   =  OUI  , /* Contenu de la table précédente                         */
...
CONT      =  OUI  , /* Si CONT = OUI des contrôles sont effectués             */
...
put @10 '                    *                     ' ;
put @10 '******************************************';
put @10 '*** Vous utilisez le programme CALMAR  ***';
put @10 '*** Copyright INSEE - décembre 1993    ***';
put @10 '******************************************' ;
put @10 '                    *                     ' ;
...
DATA _NULL_;
  ...
  put   @2 "Notes SAS                           NOTES     =  %upcase(&notes)";
   ...
   /*************************************************************************
    ***  contrôles lorsque l'on veut conserver les pondérations finales   ***
    *************************************************************************/
...
%end;
   ...
   /*************************************************************************
    ***  Premiers contrôles (facultatifs) sur les paramètres de la macro  ***
    *************************************************************************/

%if %upcase(&cont)=OUI %then
%do;

   /*  contrôles sur le paramètre DATA  */
...
%end;
  ...
  PROC SORT data=__NOMVAR;
   ...
   /**************************************************************************
    ***  Suite des contrôles (facultatifs) sur les paramètres de la macro  ***
    *************************************************************************/

%if %upcase(&cont)=OUI %then
%do;

   /*  contrôles sur le paramètre DATAMAR  */
  ...
  %end;
   ...
   /*  contrôle sur le paramètre EFFPOP  */
  ...
  %end;
   ...
   /*  contrôles sur les variables de la table DATA  */
  ...
  %end;
   ...
   /*  contrôles sur les variables de la table DATAMAR  */
...
%if &nmax=0 %then %let nmax=1;
   ...
   /*  Encore un contrôle sur les variables de la table DATAMAR  */
...
DATA __MAR1;
  ...
  %end;
   ...
   /*  Si CONT vaut OUI, des contrôles sont effectués  */
  ...
  %do;
    PROC PRINT data=__MAR1(where=(type="C"));
      ...
      title4 "ERREUR : pour au moins une variable catégorielle, les marges"
             " MAR1 à MARN,";
      title5 " o— N est le nombre de modalités, ne sont pas"
            " toutes renseignées";
    run;
  %end;
...
%end;
   ...
   /*  Un nouveau contrôle ... sur le paramètre POIDS cette fois-ci  */
...
run;
   ...
   /*  contrôle sur les effectifs des modalités des variables catégorielles   */
...
%do;
  %put ********************************************************************;
  %put ***   Le calage ne peut être réalisé. Pour rendre le calage      ***;
...
%do;
  DATA _NULL_;
    ...
    put   @10 "***   le calage ne peut être réalisé               ***";
    put   @10 "******************************************************";
...
%do;
  DATA _NULL_;
    file print;
 put //@5 "*******************************************************************";
 put @5   "***   Le calage ne peut être réalisé. Pour rendre le calage     ***";
...
DATA __CALAGE;
  ...
  poineg=(__wfin<0);
   ...
   /*  Cas o— il peut exister des poids négatifs  */
...
run;
   ...
   /*  S'il y a des poids négatifs, la variable __WFIN doit être rétablie  */
...
DATA _NULL_;
  ...
  put   @20 "*********************";
  put @2 "*";
  put @2 "*   Date : &num &mois 19&an" @40 "Heure : &systime";
  put @2 "*";
  ...
  %do;
    ...
    %else
    %do;
      put @2 "*   Le calage n'a pu être réalisé qu'approximativement"
             " en &niter itérations";
    %end;
    %if &poineg=1 %then
    %do;
  ...
  %do;
    ...
    %else
    %do;
      put @2 "*   Le calage n'a pu être réalisé";
    %end;
  %end;
